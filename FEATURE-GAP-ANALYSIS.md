...
2. **但有 5 个问题必须先修**，其中 2 个会**丢数据**、1 个会**泄露 2FA 种子**、1 个会**把域名发给第三方**。这些不是"缺功能"，是"会出事的 bug"，优先级高于任何新功能。

3. **性价比最高的补齐项不是"照抄手机版"**。手机版有大量 macOS 上**没有对应物**的能力（系统级自动填充服务、截图防护、iOS 26 CXP 导出）。真正值得做的是几个桌面端也能做、成本很低的项：TOTP 动态码、手动同步、Vault 超时设置、复制条目、清空回收站。
...
| 生成器历史 | ✅ | ❌ | |
| 密码强度计（zxcvbn） | ✅ | ❌ | |
| **导入**（LastPass/1Password/Chrome/CSV/JSON…） | ✅ | ❌ | |
| **导出**（JSON / CSV / 加密 JSON / ZIP） | ✅ | ❌ | |
| Vault 健康报告：暴露的密码 | ✅（付费） | ❌ | 本地跑，k-匿名 |
| Vault 健康报告：重复使用的密码 | ✅（付费） | ❌ | 本地跑 |
| Vault 健康报告：弱密码 | ✅（付费） | ❌ | 本地跑，zxcvbn |
| Vault 健康报告：不安全网站 | ✅（付费） | ❌ | 本地跑 |
| Vault 健康报告：未启用 2FA | ✅（付费） | ❌ | 需 2fa.directory 数据 |
| Vault 健康报告：数据泄露（HIBP） | ✅（免费） | ❌ | 需 HIBP key（自建需自购） |
| Send | ✅ | ❌ | |
| SSH Agent | ✅ 桌面端 | ❌ | SSH 密钥条目已有，但无 agent |
...
| 功能 | 官方 | Prizm | 说明 |
|---|---|---|---|
| 剪贴板自动清空 | ✅ 可配置 | ⚠️ | 固定 30 秒，**不可配置** |
| Vault 超时时长设置 | ✅ | ❌ | |
| 超时动作（锁定 / 登出） | ✅ | ❌ | |
| 主密码 re-prompt 强制执行 | ✅ | ❌ | 见 3.2 |
| 证书固定 / 自定义 CA | ✅ | ❌ | 自建常用自签证书 |
| 服务端 API / identity / icons 覆盖 | ✅ | ⚠️ | 模型支持，无 UI，且硬编码 nil |
| 截图防护 | ✅ 手机 | — | macOS 无对应物 |
| 设置项总数 | 数十项 | **2 项** | 语言 + 生物识别 |
| 界面语言 | 多语言 | ✅ en / zh-Hans | 运行时切换，292 键对齐 |
...
阶段 2  数据主权与安全工具
        └ 导出 / 导入 · 健康报告 · 强度计 · re-prompt · 证书固定
          生成器历史 · 密码历史 · 2FA 扩展 · 账号指纹短语
