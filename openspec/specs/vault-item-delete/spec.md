...
> **Note on auto-purge copy**: Bitwarden (official cloud) permanently deletes trashed items after 30 days server-side. Vaultwarden (self-hosted) does **not** auto-purge by default — it requires the admin to set `TRASH_AUTO_DELETE_DAYS`, and the period is configurable. Because Vitrine targets both servers, the supporting text intentionally omits the "30 days" timeframe to avoid a misleading promise on self-hosted instances.
