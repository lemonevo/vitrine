...
The system SHALL display a `NavigationSplitView` with a sidebar (categories + counts), a middle item
list, and a detail pane. The sidebar SHALL be organised into sections: *Menu Items* (All Items,
Favorites), *Folders* (section header always visible; folder rows shown when folders exist), *Types*
(Login, Card, Identity, Secure Note, SSH Key), *Organizations* (section visible only when the user
belongs to at least one org; each org is a disclosure group containing its collections), and *Trash*,
each with a live item count. Soft-deleted items (Trash) SHALL be excluded from all non-Trash views.

The sidebar SHALL host the vault search field, so that searching and choosing where to search happen in
the same column.
...
#### Scenario: Item rows are tinted by type
- **WHEN** the item list renders
- **THEN** each row SHALL show its icon inside a chip tinted with the colour for that item type, so the types are distinguishable without reading the subtitles
...
#### Scenario: The search field lives in the sidebar
- **WHEN** the vault browser opens
- **THEN** the search field SHALL be presented in the sidebar column
- **AND** typing in it SHALL filter the item list exactly as before
