...
**Vitrine then discards it.** `finalizeSession` never reads the field, and the next login always sends
...
- **An expiry.** The token's lifetime is the server's; Vitrine stores it and replays it until sign-out.
...
- **An "On app restart" vault timeout.** Vitrine already locks at launch, unconditionally: any stored
...
- **Making the lock-on-sleep/screensaver/screen-lock behaviour configurable.** The official client
  treats "On system lock" and "On system sleep" as choices; Vitrine locks on all three unconditionally
