...
The content pane toolbar SHALL contain a sort control in addition to the new-item button, and the
item list SHALL be displayed in the selected sort order rather than always alphabetically. The
default order remains name-ascending, so an unchanged installation behaves exactly as before.
...
#### Scenario: Item list is sorted alphabetically by default
- **WHEN** any category is selected and no sort order has been chosen
- **THEN** the item list is sorted alphabetically by item name, case-insensitive

#### Scenario: Item list follows the selected sort order
- **GIVEN** the user has chosen a sort order other than the default
- **WHEN** any category is selected
- **THEN** the item list SHALL be displayed in that order
- **AND** the sort control SHALL indicate the active order
