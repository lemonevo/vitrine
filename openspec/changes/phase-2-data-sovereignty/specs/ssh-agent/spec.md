...
Vitrine already stores SSH private keys (`SSHKeyContent.privateKey`) but offers no way to use them.
This is the only capability in this change that no other Vitrine feature competes with, and it is the
...
the socket itself SHALL NOT be world-accessible. Vitrine SHALL NOT reuse or overwrite a socket path it
...
- **THEN** Vitrine reports that the agent is unavailable **and why**
...
### Requirement: Vitrine SHALL answer identity listing

Vitrine SHALL answer `SSH_AGENTC_REQUEST_IDENTITIES` with the SSH keys in the vault that it can
actually use, and SHALL omit the ones it cannot.

#### Scenario: a key Vitrine cannot use is not offered
...
- **AND** Vitrine names the omitted one, and why, in Settings rather than leaving it unexplained
...
- **WHEN** a client asks Vitrine to sign with `K`
- **THEN** Vitrine presents the master-password prompt naming `K` and, where it can be determined,
...
- **WHEN** a client asks Vitrine to sign with `K` again
...
### Requirement: Vitrine SHALL support the key formats it can, and say which it cannot

Vitrine SHALL use unencrypted OpenSSH-format (`openssh-key-v1`) ed25519 and RSA private keys. Any
...
> Vitrine stores no passphrase for an SSH key, so there is nothing to decrypt it with. Offering the
...
### Requirement: Vitrine SHALL implement only the protocol it needs

Vitrine SHALL answer `SSH_AGENTC_REQUEST_IDENTITIES` and `SSH_AGENTC_SIGN_REQUEST`, and SHALL answer
...
- **WHEN** a client sends a message Vitrine does not implement
...
> Neither is an error condition Vitrine needs to log as a fault.

### Requirement: Vitrine SHALL NOT retain key material between requests
...
- **THEN** Vitrine holds no parsed private key
