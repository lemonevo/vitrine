...
Software Licence. Vitrine does not model it:
...
saving it in Vitrine, and an export writes it out as Generic as well. This is the same shape as the SSH
...
- **Changing the wire format.** All four values are already strings and integers on the wire; this
  change makes Vitrine stop dropping them, not invent a new encoding.
- **Validating the card fields.** A picker prevents most typos, but a value that arrives from the
  server and is not a brand or a valid month is preserved rather than rejected. Vitrine is not the
...
- **Migrating existing items.** Nothing to migrate: the values live on the server, and this change
  makes Vitrine stop erasing them on the next save.
...
integers Vitrine writes, so a wrong mapping fails loudly rather than silently relabelling a passport as
