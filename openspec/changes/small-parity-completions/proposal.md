...
username; Vitrine's offers the first two (`PasswordGeneratorConfig.Mode`, `:7-10`). A password manager
...
deleted automatically. Vitrine's shows a list, a per-row context menu and an "Empty Trash" bar
...
- **Naming a retention period.** Bitwarden's purge interval is a **server** setting, and Vitrine
  implements no endpoint that reads it. Printing "30 days" would be inventing a number for a server
  Vitrine cannot see — and worse than saying nothing, because a user would plan around it. The notice
