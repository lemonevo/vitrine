...
This is the initial release of Macwarden — a native macOS Bitwarden/Vaultwarden client built entirely with SwiftUI and Apple frameworks.
...
- **Data**: Native Bitwarden crypto (`BitwardenCryptoServiceImpl` actor using CommonCrypto + CryptoKit + Argon2Swift), `MacwardenAPIClient` (URLSession), `VaultRepositoryImpl`, `CipherMapper`, `KeychainService`, `FaviconLoader`.
