...
Users can browse and edit existing vault items but cannot create new ones. To save a new login, card, identity, secure note, or SSH key, users must use another Bitwarden client. Adding item creation completes the core CRUD lifecycle and makes Prizm a self-sufficient vault manager.
...
- Add a `POST /api/ciphers` endpoint to `PrizmAPIClientProtocol`
...
- `PrizmAPIClientProtocol` / `PrizmAPIClientImpl` — new `createCipher` method
